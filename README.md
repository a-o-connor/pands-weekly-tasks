...
5. The folder task_week06 contains a Jupyter notebook square_root.ipynb, written in Python. In it, a function is defined that uses Newton's square root finding algorithm to return the square root of a user input number. 
6. The folder task_week07 contains three files. The first is a Lorem Ipsum text file. The Jupyter notebook, week07_task.ipynb, contains my initial attempts to create a function that will read in a text file and output the number of e's it contains. However, the final assignment specified that the filename should be taken from an argument on the command line. Therefore, in order for the command line argument to be accepted, the final assignment is contained in a Python file called ecounter.py. 
